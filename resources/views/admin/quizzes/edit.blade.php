<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center space-x-4">
            <a href="{{ $quiz->lesson_id ? route('admin.lessons.slides.index', $quiz->lesson_id) : route('admin.topics.show', $quiz->topic_id) }}" class="p-3 bg-slate-100 dark:bg-white/5 rounded-2xl hover:bg-slate-200 dark:hover:bg-white/10 transition">
            ...
            <div>
                <h2 class="font-black text-3xl text-slate-900 dark:text-white leading-tight tracking-tighter uppercase italic">
                    Edit <span class="text-red-600">Evaluasi</span>
                </h2>
                <p class="text-slate-500 dark:text-slate-400 text-sm font-medium">Modifikasi tantangan ujian untuk mahasiswa.</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-[#1e293b] overflow-hidden shadow-2xl rounded-[2.5rem] border-2 border-slate-100 dark:border-white/5 p-8 md:p-12 relative group">
                <form method="POST" action="{{ route('admin.quizzes.update', $quiz) }}" class="space-y-8 relative z-10">
                    @csrf
                    @method('PATCH')
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="3" class="block mt-1 w-full bg-slate-50 dark:bg-slate-900/50 border-2 border-slate-100 dark:border-white/5 focus:border-blue-500 focus:ring-0 rounded-2xl px-4 py-3 text-slate-900 dark:text-white transition duration-200 font-bold">{{ old('description', $quiz->description) }}</textarea>
