        ...
        <div class="flex items-center space-x-4">
            ...
            <div>
                <h2 class="font-black text-3xl text-slate-900 dark:text-white leading-tight tracking-tighter uppercase italic">
                    Ekspansi <span class="text-blue-600">Wilayah</span>
                </h2>
                <p class="text-slate-500 dark:text-slate-400 text-sm font-medium">Ciptakan dunia baru untuk para pahlawan.</p>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-[#1e293b] overflow-hidden shadow-2xl rounded-[2.5rem] border-2 border-slate-100 dark:border-white/5 p-8 md:p-12 relative group">
                <div class="absolute top-0 right-0 p-8 opacity-5">
                    <i class='bx bx-map-pin text-9xl'></i>
                </div>

                <form method="POST" action="{{ route('admin.topics.store') }}" class="space-y-8 relative z-10">
                    @csrf

                    <!-- Title -->
                    <div>
                        <x-input-label for="title" :value="__('Nama Wilayah')" />
                        <x-text-input id="title" class="block mt-1 w-full" type="text" name="title" :value="old('title')" required autofocus placeholder="Contoh: Integral Tentu" />
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div>
                            ...
                            <x-text-input id="required_level" class="block mt-1 w-full" type="number" name="required_level" :value="old('required_level', 1)" required />
