<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center space-x-4">
            <a href="{{ route('admin.topics.show', $topicId) }}" class="p-3 bg-slate-100 dark:bg-white/5 rounded-2xl hover:bg-slate-200 dark:hover:bg-white/10 transition">
            ...
            <div>
                <h2 class="font-black text-3xl text-slate-900 dark:text-white leading-tight tracking-tighter uppercase italic">
                    Tambah <span class="text-blue-600">Materi</span>
                </h2>
                <p class="text-slate-500 dark:text-slate-400 text-sm font-medium">Buat materi baru untuk mahasiswa.</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-[#1e293b] overflow-hidden shadow-2xl rounded-[2.5rem] border-2 border-slate-100 dark:border-white/5 p-8 md:p-12 relative">
                <form method="POST" action="{{ route('admin.lessons.store') }}" class="space-y-8">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div>
                            ...
                            <x-text-input id="xp_reward" class="block mt-1 w-full" type="number" name="xp_reward" :value="old('xp_reward', 10)" required />
                        </div>

                        <!-- Order -->
                        <div>
                            <x-input-label for="order" :value="__('Urutan')" />
                            <x-text-input id="order" class="block mt-1 w-full" type="number" name="order" :value="old('order', 1)" required />
                        </div>
                    </div>
