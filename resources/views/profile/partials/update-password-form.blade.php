<section>
    <header>
        ...
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
            {{ __('Pastikan akunmu menggunakan kata sandi acak yang panjang agar tetap aman.') }}
        </p>
    </header>
    ...
    <form method="post" action="{{ route('password.update') }}" class="mt-6 space-y-6">
        ...
        <div class="flex items-center gap-4">
            <x-primary-button>{{ __('Perbarui Mantra') }}</x-primary-button>
            ...
            @if (session('status') === 'password-updated')
                ...
                ><i class='bx bx-check-double'></i> {{ __('Kata Sandi Berhasil Diubah.') }}</p>
            @endif
        </div>
    </form>
</section>
